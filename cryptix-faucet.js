...
class CryptixFaucet extends EventEmitter {
  constructor(appFolder) {
    super();
    this.appFolder = appFolder;
    this.config = utils.getConfig(
      path.join(appFolder, "config", "cryptix-faucet"),
    );
...
*/
  ...
  calculateAvailable({ network, ip, address }) {
    ...
    const period = transactions.length
      ...
      : null;
    log.info(
      `Address ${address} on ip ${ip} has ${available / 100000000} CYTX available`,
    );
  ...
  async initFaucet() {
    ...
    (async () => {
      for await (const msg of requests) {
        ...
        log.info(`request from ${ip} for ${Wallet.CYTX(amount)}`);
        const limit =
    ...
    const getAvailable = async ({ network, ip }) => {
      if (!network || !/^cryptix(test|dev|sim)*/.test(network))
    ...
    const getCryptix = async ({ address, amount: amount_, ip }) => {
      const amount = parseInt(amount_);
      if (isNaN(amount) || !amount || amount < 0)
        return { error: `Invalid amount: ${amount_}` };

      if (!address || !/^cryptix(test|dev|sim)*:/.test(address))
      ...
      if (available < amount) {
        return {
          error: `Unable to send funds: you have ${this.CYTX(available)} CYTX ${period == null ? `available.` : `remaining. Your limit will update in ${this.duration(period)}.`}`,
        };
      } else {
        try {
          const fee = 0;
    ...
    /*
		...
		this.flowHttp.app.get("/api/:apikey/get/:address", async (req,res) => {
			...
			const ip = req.query.ip;
			res.json(await getCryptix({address, amount, ip}))
		});
		*/
  ...
  async main() {
    ...
    program
      ...
      .option("--limit <limit>", `CYTX/day limit per IP`, (limit) => {
        limit = parseFloat(limit);
        if (isNaN(limit) || limit <= 0)
          throw new Error("CYTX/day limit is invalid");
        return limit;
      })
      .option("--no-limit", "disable CYTX/day limit")
...
(async () => {
  let cryptixFaucet = new CryptixFaucet(__dirname);
  try {
    await cryptixFaucet.main();
  } catch (ex) {
