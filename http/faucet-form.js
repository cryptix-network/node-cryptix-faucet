...
export class FaucetForm extends BaseElement {
  ...
  render() {
    ...
    return html`
      <div class="message" style="color: #00bcd4 !important";>
        Enter your address and the amount of Cryptix you want to receive:
      </div>
      <flow-input
      ...
      ></flow-input>
      <flow-input label="Amount (CYTX)" class="amount" value=""></flow-input>
      <flow-select
        label="Network"
      ...
      <flow-btn primary @click="${this.submit}">CLAIM COINS</flow-btn>
  ...
  submit() {
    ...
    //cryptixtest:qq0nvlmn07f6edcdfynt4nu4l4r58rkquuvgt635ac
    console.log({ address, network, amount, captcha });

    if (!/^cryptix:[1-9A-HJ-NP-Za-km-z]/.test(address)) {
    ...
    flow.app.rpc.request(
      ...
      (error, result) => {
        ...
        if (error) {
          ...
          if (error.error == "limit") {
            ...
            if (/ApiError/.test(msg)) {
              msg = html`<div class="api-error" style="color: #000 !important;">
                ${[msg].map((v) => html`${v}<br />`)}
              </div>`;
            }
          }
          ...
          FlowDialog.show({
            title: html`<b class="error" style="color: #000 !important;">Error</b>`,
            body: html` <div class="msg" style="color: #000 !important;">${msg}</div> `,
            cls: "custom",
        ...
        FlowDialog.show({
          title: "Success",
          body: html`
            <div class="msg" style="color: #000 !important;">
              We have successfully sent
              <b>${CYTX(result.amount)} CYTX</b> to the requested address:<br />&nbsp;<br />
              <b>${address}</b><br />&nbsp;<br />
              <span class="txid" style="color: red !important;"><nobr>TXID: ${result.txid}</nobr></span>
            </div>
          `,
          cls: "custom",
          btns: ["Close:primary:close"], //, {text:"Ok", cls:"success", value:"ok-btn"}]
